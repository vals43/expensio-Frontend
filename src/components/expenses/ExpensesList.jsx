...
const ExpenseList = ({ onEditExpense }) => {
  ...
  const { handleDeleteExpense } = useExpenseActions()
  ...
  const getExpenseTypeBadge = (expense) => {
    const isRecurring = expense.type === "recurring" || (expense.startDate && expense.startDate !== expense.date)
    return {
  ...
  const formatRecurringDates = (expense) => {
    const isRecurring = expense.type === "recurring" || (expense.startDate && expense.startDate !== expense.date)
    if (!isRecurring) return null
    const startDate = formatDate(expense.startDate || expense.date)
    const endDate = expense.endDate ? formatDate(expense.endDate) : "Ongoing"
  ...
  const filteredTransactions = useMemo(() => {
    ...
    const result = expenses.filter((t) => {
      const matchesSearch =
  ...
  const handleOpenReceipt = async (expenseId) => {
    try {
      await openReceipt(expenseId, setSelectedReceipt)
    } catch (err) {
            ...
            {filteredTransactions.map((t) => (
              <motion.div key={t.id} initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }}>
                <Card className="hover:shadow-2xl shadow-xl border-l-4 border-b-4 border-gray-200 dark:border-gray-700 transition-all duration-300 hover:scale-[1.02] rounded-xl">
                    ...
                    <div className="flex items-start justify-between">
                      <div className="flex-1">
                        ...
                        <div className="flex items-center gap-2 mt-2">
                          ...
                          <span
                            className={`px-2 py-1 text-xs font-medium rounded-full ${getExpenseTypeBadge(t).className}`}
                          >
                            {getExpenseTypeBadge(t).label}
                          </span>
                        </div>
                      </div>
                      <CardAction>
                        <div className="flex items-center gap-1">
                          {t.receipt && (
                            <Button variant="outline" size="sm" onClick={() => handleOpenReceipt(t.id)}>
                              View Receipt
                            </Button>
                          )}
                          <Button variant="ghost" size="sm" onClick={() => onEditExpense(t)}>
                  ...
                  <CardContent>
                    {!getExpenseTypeBadge(t).label.includes('Recurring') && (
                      <div className="flex items-center gap-2 text-sm">
                        <CalendarIcon className="h-4 w-4" />
                        <span>{formatDate(t.date)}</span>
                      </div>
                    )}
                    {formatRecurringDates(t) && (
                      <div className="flex items-center gap-2 text-sm mt-2 text-blue-600 dark:text-blue-400">
                        <CalendarIcon className="h-4 w-4" />
                        <span className="font-medium">{formatRecurringDates(t)}</span>
                      </div>
                    )}
                    {t.description && (
                  ...
                  <motion.tr key={t.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="hover:bg-muted/30">
                    ...
                    <TableCell>
                      {formatRecurringDates(t) ? (
                        ...
                        <span className="text-blue-600 dark:text-blue-400 font-medium text-sm">{formatDate(t.date)}</span>
                      )}
                    </TableCell>
              ...
              <div className="p-6 flex items-center justify-center">
                {selectedReceipt?.url ? (
                  selectedReceipt.type === "application/pdf" ? (
                    <iframe src={selectedReceipt.url} className="w-full h-[80vh]" title="Receipt PDF" />
                  ) : (
                    <img
                      src={selectedReceipt.url || "/placeholder.svg"}
                      alt="Receipt"
